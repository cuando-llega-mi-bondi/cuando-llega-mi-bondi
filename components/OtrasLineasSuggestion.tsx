...
export const OtrasLineasSuggestion = memo(function OtrasLineasSuggestion({
  ...
  return (
    <div className="mt-4 animate-slide-up rounded-xl bg-card border border-border p-4 shadow-sm">
      <div className="mb-3 flex items-center gap-2">
        ...
        <span className="font-mono text-[10px] font-semibold tracking-[1.2px] text-muted-foreground">
      ...
      {loading ? (
        <div className="flex gap-2 overflow-x-auto pb-1">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="blink h-8 w-[60px] flex-shrink-0 rounded-full bg-muted"
            />
          ))}
        </div>
      ) : (
        <div className="flex flex-wrap gap-2">
          {lineas.map((linea) => (
            <button
              ...
              className="cursor-pointer rounded-full border border-border bg-muted px-3 py-1.5 font-sans text-sm font-medium text-foreground transition hover:-translate-y-px hover:bg-secondary hover:border-secondary hover:text-secondary-foreground active:scale-95"
            >
              {linea.Descripcion}
            </button>
          ))}
